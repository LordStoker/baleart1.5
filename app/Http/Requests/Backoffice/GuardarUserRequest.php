...
class GuardarUserRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['required', 'string', 'min:5', 'max:100', Rule::unique('users')->ignore($this->route('user'))],
            'last_name' => 'required|string|min:2|max:100',
            'email' => ['required', 'email', Rule::unique('users')->ignore($this->route('user'))],
            'phone' => 'required|string|min:6|max:9',
    ...
    public function messages(){
        ...
        'phone.max' => 'El campo teléfono no puede tener más de 9 caracteres',
        ];
    }
}
